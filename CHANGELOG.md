...
`bz.Queue` gets a second member. `graph.add_pass(queue=bz.Queue.COMPUTE)` puts
a pass on the device's compute queue, where it runs beside the graphics work
instead of through it. 0.28 added the parameter and this release adds the
value, so a program written for 0.28 schedules exactly as it did.

The graph puts the waits between the two queues for you, in two places. Inside
one graph, a pass that reads what a pass on the other queue wrote waits for it.
A graph that you send again waits for its own previous submit on the other
queue.

Between two DIFFERENT graphs on different queues there is no automatic order.
That is a decision: two graphs are how you say that work may overlap. Use
`ctx.submit(graph, after=serial)` when one must follow the other. The `Serial`
a submit returns covers every queue that submit used.

`Feature.ASYNC_COMPUTE` says whether the device has a queue family that does
compute and not graphics. Without one, a `Queue.COMPUTE` pass runs on the
graphics queue under its own timeline: the same program with the same order,
and no overlap. `bz.Context(features=[bz.Feature.ASYNC_COMPUTE])` refuses a
device that cannot give you the overlap.

Uploads stay on the graphics queue, and a compute pass that reads an uploaded
image waits for it.

### Added

- **`bz.Queue.COMPUTE`**. `add_pass(queue=bz.Queue.COMPUTE)` runs a pass on the
  compute queue. A pass with a render target refuses it, because a compute
  queue has no rasterizer. `blit_image` and `generate_mipmaps` refuse it too:
  both need a graphics queue, and the message names the fix.
- **`bz.Feature.ASYNC_COMPUTE`**. True when the device has a compute-only queue
  family. `ctx.supports()` and `device.supports()` both answer it.
- **`after=` orders work across the queues**, and `ctx.wait(serial)` waits for
  every queue one submit used.
- **A `BufferType.STORAGE` buffer binds as an index buffer**, so a compute
  shader can write the indices a draw reads. `bind_index_buffer` refuses a
  vertex or uniform buffer with a message instead of leaving it to the
  validation layers.
- **HLSL `Texture2D` and `SamplerState` compile to one combined texture
  binding**, which is what `.texture()` declares. Before this an HLSL shader
  that sampled a texture built a pipeline no descriptor set could feed.

### Examples

- **45_async_compute** runs the same producer and consumer chain with the
...
- A windowed frame records its ring-slot serials. Only the headless path did,
  so work sent from a window paced nothing.
- `ctx.submit()` between `acquire()` and `present()` raises `StateError`. It
  used to advance the frame ring under the window's fence and semaphores.
- `ctx.wait()` no longer waits forever after a submit that failed. It waits for
  the last serial a queue really signalled, not for the one that was reserved.
- A `Serial` from another Context raises `ResourceError`. It used to wait for
  that number on this Context's timeline, which is a wrong answer or a stall.
- A failed read of the submission timeline is logged. It used to return 0 in
...
- Buffers and images are shared between the two queue families
  (`VK_SHARING_MODE_CONCURRENT`) when the families differ. This costs a little
  bandwidth on some hardware and removes the ownership transfer that every
  resource would otherwise need on the way across.
- The barriers bazalt writes are narrowed to the queue family that replays
  them. A compute queue supports fewer pipeline stages, and a barrier may only
  name the ones its family has.
- `renderer.gpu_time_ms` measures the graphics part of a frame. Work on the
  compute queue overlaps it rather than adding to it, so measure a compute pass
  with `p.timer()`.
- The last two items under Added do not share this release's subject. Each is
  one line plus its test, and each closed a gap that had no way around it.
