...
public:
    explicit HotReloadWatcher(Context& context)
        : context_(context),
          poll_interval_(std::chrono::milliseconds(poll_ms_from_env()))
    {
    ...
    void drain() override
    {
        ...
        for (const auto& weak : shaders_)
        {
            ...
            // The language, the include dirs and the entry point come off the
            // module: a recompile that dropped them would parse the file as the
            // other language, resolve a different #include or pick a different
            // HLSL function, and the failure would look like a shader edit gone
            // wrong rather than a reload losing its settings.
        ...
        if (!replaced.empty())
        {
            for (const auto& weak : pipelines_)
            {
                ...
                const bool affected = std::ranges::any_of(replaced, [&](ShaderModule* m) { return pipeline->uses(m); });
                if (!affected)
                {
                    continue;
                }
...
private:
    ...
    static bool shader_affected_(const ShaderModule& module, const std::set<std::string>& changed)
    {
        ...
        return std::ranges::any_of(module.includes(), [&](const std::string& inc) { return changed.contains(inc); });
    }
