...
// Which queue a pass runs on, and the vocabulary the two timelines need.
//
...
// A ternary rather than a cast: a pybind enum accepts any int, so Queue(7) must
// index something that exists rather than run off the array.
inline constexpr std::size_t queue_index(QueueKind kind)
{
    return kind == QueueKind::Compute ? 1 : 0;
}
