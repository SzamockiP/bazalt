...
- **Image Transfers:** `cmd.copy_image(src, dst)` copies one image into another of the same size and format. Use it for a history buffer, for a temporal effect, or for a compute ping-pong. `cmd.clear_image(image, color)` fills an image with no pipeline and no pass.
