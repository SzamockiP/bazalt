...
- **One Rule for What Blocks:** Every write is asynchronous and every read blocks. `create_buffer`, `create_image` and `load_image` give you the handle before the GPU has the data, and the handle is its own future. Anything that uses it waits for it, so you write no wait at all in normal code. `buf.read()`, `img.read()` and `target.color[0].read()` block, because they give you an array and have nothing to give you until the bytes arrive. One verb waits when you want to be explicit: `ctx.wait()` for everything this Context started. To wait for less, wait on the resource with `buf.wait()` or `img.wait()`.
